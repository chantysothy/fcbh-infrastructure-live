...
module "couchbase_data_nodes" {
  ...
  ami_id = data.template_file.ami_id.rendered #"ami-00b71c714ab2243cb"
  user_data = data.template_file.user_data_couchbase_data_nodes.rendered

  vpc_id     = data.aws_vpc.obt.id
  subnet_ids = data.aws_subnets.obt.ids
...
# ---------------------------------------------------------------------------------------------------------------------
# USE THE FCBH amazon linux2 AMI IF VAR.AMI_ID IS NOT SPECIFIED. 
# This AMI was created with packer
# ---------------------------------------------------------------------------------------------------------------------

data "aws_ami" "fcbh_amazon_linux2" {
  most_recent = true
  owners      = ["121033846327"] # FCBH obt-dev
  ...
  filter {
    name   = "name"
    values = ["*couchbase-amazon-linux*"]
  }
}

data "template_file" "ami_id" {
  template = var.ami_id == null ? data.aws_ami.fcbh_amazon_linux2.id : var.ami_id
}

# ---------------------------------------------------------------------------------------------------------------------
# DEPLOY COUCHBASE IN THE OBT VPC, on private subnets
# ---------------------------------------------------------------------------------------------------------------------

data "aws_vpc" "obt" {
  filter {
    name   = "cidr"
    values = ["172.10.0.0/16"]
  }
}

data "aws_subnets" "obt" {
filter {
    name   = "vpc-id"
    values = [ data.aws_vpc.obt.id]
  }  
  tags = {
    Attributes = "private"
  }  
